...
/// Vida recogible dentro de un nivel (Health Pickup).
///
/// Estado de PARTIDA puro, con la MISMA forma que `AmmoPickup` y por
/// el mismo motivo: no tiene vida, IA, pathfinding ni estados
/// `Idle`/`Alert`/`Hit`/`Dead` propios — solo recuerda DÓNDE está y
/// si sigue disponible en ESTA sesión.
///
/// No renderiza píxeles, no conoce `Framebuffer`/`TextureAsset`/
/// `TextureManager`, no decide cuánta vida otorga (eso vive en
/// `GameSession`, que sí conoce `Player`) y no realiza la
/// comprobación de distancia contra el jugador (también en
/// `GameSession`, la única capa con acceso a `Player` simultáneamente
/// con la colección de pickups).
...
impl HealthPickup {
    ...
    /// `AmmoPickup::at_cell`/`Entity::dealer_at_cell`/
    /// `Player::from_level`: no se inventa una segunda conversión
    /// cell -> world.
    ...
    /// pickups desde `Level::health_spawns` otra vez).
...
mod tests {
    ...
    fn new_pickup_is_active() {
        let pickup = HealthPickup::at_cell(1, 2, BLOCK_SIZE);
    ...
    fn deactivate_makes_it_inactive() {
        let mut pickup = HealthPickup::at_cell(0, 0, BLOCK_SIZE);
